...
// CurrentTime returns the time in UTC format
func CurrentTime() time.Time {
	return time.Now().UTC()
}

// GenTimeAgo returns time-ago string according to different rules
func GenTimeAgo(date *time.Time) []byte {
	timeAgo := CurrentTime().Sub(*date)
...
// FormatDate formats an input date string to a given format
